...
void ofxHttpUtils::start() {
        ...
        ofLog(OF_LOG_VERBOSE, "thread started\n");
}
// ----------------------------------------------------------------------
    ...
    while( isThreadRunning() == true ){
    	...
    	if(forms.size()>0){
			...
			if(form.method==OFX_HTTP_POST){
				doPostForm(form);
				ofLog(OF_LOG_VERBOSE, "ofxHttpUtils: (thread running) form submitted (post):"+form.name);
			}else{
				string url = generateUrl(form);
				ofLog(OF_LOG_VERBOSE, "ofxHttpUtils: (thread running) form submitted (get): " +form.name);
				getUrl(url);
			}
    		lock();
			forms.pop();
