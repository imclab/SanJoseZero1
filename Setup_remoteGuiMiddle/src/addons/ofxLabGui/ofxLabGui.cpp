...
//-----------------------------
void ofxLabGui::setDimensions(float width, float height){
	hitArea.width       = width;
	hitArea.height      = height;
	boundingBox.width   = width;
	boundingBox.height  = height;
	for (int i=0; i<panels.size(); i++){
		panels[i]->setDimensions(boundingBox.width, boundingBox.height - boundingBox.y - topBarHeight - borderWidth*2 );
	}
}
...
//---------------------------------------------
void ofxLabGui::setPanelIndex(string panelName, int newIndex){
	int oldIndex = -1;
	for(int i = 0; i < panels.size(); i++){
        if( panels[i]->name == panelName){
			oldIndex = i;
	...
	guiTypePanel * toSwap = panels[newIndex];
	guiTypePanel * toMove = panels[oldIndex];
	panels[oldIndex] = toSwap;
	panels[newIndex] = toMove;
	
	//re-sort tabs
	
	for (int i=0; i<panels.size(); i++){
		if (i > 0){
			ofRectangle tabRect = panels[i-1]->getTabRect();
			panels[i]->setTabPosition(tabRect.x + tabRect.width, tabRect.y);
		} else {
			ofRectangle tabRect = panels[i]->getTabRect();
			tabRect.x = tabRect.y = 0;
			//panelPtr->setTabPosition(borderWidth, topSpacing/2);
			panels[i]->setTabPosition(0,0);
...
guiTypeMultiSelectToggle * ofxLabGui::addMultiSelectToggle(string name, string xmlName, vector <bool> defaultSelected, vector <string> boxNames){
	if( currentPanel < 0 || currentPanel >= panels.size() )return NULL;
	
	//add a new multi toggle to our list
	guiTypeMultiSelectToggle * tmp = new guiTypeMultiSelectToggle();
	
	//setup and dimensions
	tmp->setup(name, defaultSelected, boxNames);
	tmp->setDimensions(180, 60);
	tmp->xmlName = xmlName;
	...
	xmlObjects.push_back( xmlAssociation(tmp, xmlName, defaultSelected.size()) );
	guiObjects.push_back(tmp);
...
guiTypeSlider * ofxLabGui::addSlider(string sliderName, string xmlName, float value , float min, float max, bool isInt){
    ...
    tmp->setDimensions(panels[currentPanel]->boundingBox.width - borderWidth*6, 10);
    tmp->xmlName = xmlName;
...
//---------------------------------------------
int ofxLabGui::getValueType(string xmlName){
    for(int i = 0; i < guiObjects.size(); i++){
        if( guiObjects[i]->xmlName == xmlName){
			return guiObjects[i]->dataType;
        }
    }
	return -1;
}
...
//---------------------------------------------
int ofxLabGui::getNumValues(string xmlName){
    for(int i = 0; i < guiObjects.size(); i++){
        if( guiObjects[i]->xmlName == xmlName){
			return guiObjects[i]->value.getNumValues();
        }
    }
	return 0;
}
...
bool ofxLabGui::getValueB(string xmlName, int whichParam){
    ...
    ofLog(OF_LOG_WARNING, "ofxLabGui - paramater requested %s %i doesn't exist - returning 0", xmlName.c_str(), whichParam);
    return 0;
}
...
//---------------------------------------------
guiValue * ofxLabGui::getValue(string xmlName, int whichParam){
	for(int i = 0; i < xmlObjects.size(); i++){
        if( xmlObjects[i].guiObj != NULL && xmlName == xmlObjects[i].xmlName ){
            if( whichParam >= 0 && whichParam < xmlObjects[i].numParams ){
                return &xmlObjects[i].guiObj->value;
            }
        }
    }
    ofLog(OF_LOG_WARNING, "ofxLabGui - paramater requested %s %i doesn't exist - returning blank value", xmlName.c_str(), whichParam);
    return new guiValue();
};
...
void ofxLabGui::loadSettings(string xmlFile){
	...
	if (!loaded) ofLog(OF_LOG_WARNING, "xml load failed! "+currentXmlFile);
    usingXml = true;
	
	bool bHasSettingsTag = settings.pushTag("settings");
	...
	if (bHasSettingsTag) settings.popTag();
}

//-----------------------------
void ofxLabGui::loadSettings(ofxXmlSettings _settings, string xmlFile){
	cout<<"LOADING "<<xmlFile<<endl;
    for(int i = 0; i < guiObjects.size(); i++)guiObjects[i]->loadSettings(xmlFile);
...
void ofxLabGui::saveSettings(string xmlFile){
	...
	bool pushSettings = settings.pushTag("settings");
...
void ofxLabGui::mousePressed(float x, float y, int button){
	...
	if( bCollapsible && isInsideRect(x, y, minimizeButton)){
        ...
        for(int i = 0; i < panels.size(); i++){
            ...
            if( isInsideRect(x- boundingBox.x, y, scaledTabRect) && !bPanelClicked){
                selectedPanel = i;
				panels[i]->bSelected = true;
	...
	if( isInsideRect(x, y, topBar) && bDraggable){
        dragging = true;
					...
					for (int k=0; k<panels[i]->lastUpdated.size(); k++){
						if (panels[i]->lastUpdated[k]->lastUpdated.size() > 0){
							for (int j=0; j<panels[i]->lastUpdated[k]->lastUpdated.size(); j++){
								...
								cout<<"let's send "<<panels[i]->lastUpdated[k]->lastUpdated[j]->xmlName<<endl;
...
//--------------------------------------------------------------
void ofxLabGui::draw( string whichPanel ){
	for(int i = 0; i < panels.size(); i++){
...
//--------------------------------------------------------------
void ofxLabGui::draw( int whichPanel ){
	if (whichPanel > panels.size()-1) {
		ofLog(OF_LOG_ERROR, "Panel does not exist!");
		return;
	};
	...
	// draw bg if selected; otherwise just drawing tab
	if (selectedPanel == whichPanel){
	...
	if( !minimize ){		
		glPushMatrix();{
			glTranslatef(hitArea.x, hitArea.y, 0);			
			if( selectedPanel == whichPanel )panels[whichPanel]->render();
			else panels[whichPanel]->renderTab();
		} glPopMatrix();
	}
