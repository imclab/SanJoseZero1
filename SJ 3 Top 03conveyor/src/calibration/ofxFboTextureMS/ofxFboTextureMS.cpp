...
//static vars
bool ofxFBOTextureMS::bSupportChecked = false;
bool ofxFBOTextureMS::bSupportsFBO;
bool ofxFBOTextureMS::bSupportsBlit;
bool ofxFBOTextureMS::bSupportsMulti;
GLint ofxFBOTextureMS::maxTextureSize;
GLint ofxFBOTextureMS::maxRenderBufferSize;
GLint ofxFBOTextureMS::maxSamples;

ofxFBOTextureMS::ofxFBOTextureMS() {
	screenFov		= 60;
	oldFramebuffer	= 0;
	//	oldRenderbuffer = 0;
	pixels			= NULL;
...
void ofxFBOTextureMS::allocate(int w, int h, int internalGlDataType, int numSamples) {
	_isActive = false;
	...
	if(!bSupportsFBO){
		ofLog(OF_LOG_ERROR, "ofxFBOTextureMS: GL_EXT_framebuffer_object not supported by current OpenGL renderer. "
							" For Apple machines, more info at http://developer.apple.com/graphicsimaging/opengl/capabilities");
		return;
	}
	
	if(!bSupportsMulti && numSamples){
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS: GL_EXT_framebuffer_multisample not supported by current OpenGL renderer."
	...
	if(!bSupportsBlit && numSamples){
		...
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS: GL_EXT_framebuffer_blit not supported by current OpenGL renderer."
			  " Falling back to non-multisampled mode. To disable this message, specify 0 samples."
			  " Apple machines, more info at http://developer.apple.com/graphicsimaging/opengl/capabilities");
		numSamples = 0;
	}
	...
	if(maxSamples < numSamples){
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS: requested samples too high. Using GL_SAMPLES instead.");
	...
	if(maxTextureSize < w || maxRenderBufferSize < w){
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS: requested width was too large. Using largest allowed value instead.");
	...
	if( maxTextureSize < h || maxRenderBufferSize < h){
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS: requested height was too large. Using largest allowed value instead.");
	...
	if(internalGlDataType==0){
		ofLog(OF_LOG_WARNING, "ofxFBOTextureMS::allocate( _ , _ , HERE , _ ) "
    ...
    //if (GLEE_ARB_texture_rectangle){//CHNGED BY LARS
        texData.tex_w = w;
        texData.tex_h = h;
	...
	if(status != GL_FRAMEBUFFER_COMPLETE_EXT) {
		...
		if(internalGlDataType != GL_RGBA &&
		   ...
		   internalGlDataType != GL_RGBA32F_ARB){
			...
			ofLog(OF_LOG_ERROR,  "ofxFBOTextureMS: Failed to initialize. "
				  ...
				  "ofxFBOTextureMS::allocate( _ , _ , HERE , _ ) ");
...
void ofxFBOTextureMS::setupScreenForMe(){
...
void ofxFBOTextureMS::setupScreenForThem(){
	//      glPopAttrib();
	//      return;
...
void ofxFBOTextureMS::clear(float r, float g, float b, float a) {
    clearColor[0] = r;
    clearColor[1] = g;
    clearColor[2] = b;
    clearColor[3] = a;
...
void ofxFBOTextureMS::bindAsTexture(){
	glBindTexture(GL_TEXTURE_2D, (GLuint)&texData.textureID);
}

void ofxFBOTextureMS::begin() {
	swapIn();
	setupScreenForMe();
}

void ofxFBOTextureMS::end() {
	swapOut();
	setupScreenForThem();
}
...
void *ofxFBOTextureMS::getPixels() {
...
bool ofxFBOTextureMS::isExtensionSupported(const char *extension){
...
void ofxFBOTextureMS::supportCheck(){
	if(!bSupportChecked){
