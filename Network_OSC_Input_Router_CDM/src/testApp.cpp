...
void testApp::setup(){
	...
	settings.pushTag("misc");{
		...
		logDelayThreshold = settings.getValue("logdelay",logDelayThreshold);
	} settings.popTag();
	...
	// Setup the data logging class
	curEmitID = -1;
	string url = "http://www.plug-in-play.com/php/cdmLogger.php";
	dataLogger = new Logger(logDelayThreshold, url);
}
		...
		for (int i=0; i<emitters.size(); i++){
			if (emitters[i]->checkMessageString(m.getAddress())){
				...
				if (bCanEmit){
					...
					// ZACK BOKA
					// Lastly, log that a particle was emitted (only if the log delay threshold has been met)
					dataLogger->logData();
