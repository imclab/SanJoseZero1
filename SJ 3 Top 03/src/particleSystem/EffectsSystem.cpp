...
void EffectsSystem::update(){
	...
	for(int i=0;i<emitter->particles.size();i++)
	{
		if(emitter->particles[i]->bMagnifying){
			addToFluid(Vec2f((ofRandomf()*emitter->particles[i]->getWidth()/2+ emitter->particles[i]->getLoc().x)/ofGetWidth(), (ofRandomf()*emitter->particles[i]->getHeight()/2+emitter->particles[i]->getLoc().y)/ofGetHeight()), Vec2f(emitter->particles[i]->vel.normalized().x/6.0f, emitter->particles[i]->vel.normalized().y/6.0f), true, true);
		}
	}
