...
void testApp::setup(){
	...
	if (mode == MIC){
		...
		song.loadSound(songToLoad, true);
		song.play();
		song.setLoop(true);
		
		// this describes an amplification filter
		for(int i = 0; i < fft->getBinSize(); i++)
			eqFunction[i] = 0.5;//(float) (fft->getBinSize() - i) / (float) fft->getBinSize();
...
void testApp::loadSettings(){	
	...
	if(XML.loadFile("settings.xml"))
	{
		...
		mode = XML.getValue("prefs:mode", 0);
		songToLoad = XML.getValue("prefs:song", "song.mp3");
		
		host = XML.getValue("osc:host", "localhost");
		port = XML.getValue("osc:port", 12000);
		...
		for(int i=0;i<numTriggers;i++)
		{
			...
			// ZACK BOKA: Load message addresses
			XML.pushTag("Trigger", i); {
				int numAddresses = XML.getNumTags("messageString");
				for (int j = 0; j < numAddresses; j++) {
					newTrigger.addresses.push_back(XML.getValue("messageString","error",j));
				}
			} XML.popTag();
...
void testApp::saveSettings()
{
	...
	if (!XML.tagExists("prefs")) XML.addTag("prefs");
	if(sendFullSpectrum)
		XML.setValue("prefs:Send_Full_Spectrum", "true", 0);
	else {
		XML.setValue("prefs:Send_Full_Spectrum", "false", 0);
	}
	XML.setValue("prefs:mode", mode);
	XML.setValue("prefs:song", songToLoad);
	
	for(int t=0;t<triggers.size();t++)
	{
		if (!XML.tagExists("Trigger", t)) XML.addTag("Trigger");
...
void testApp::update(){
	...
	if (mode == MP3){
		//get FFT from mp3 if mode is MP3
		//update fft
		float * input = ofSoundGetSpectrum(bufferSize*2);
		...
		{
			for(int i = 0; i < fft->getBinSize(); i++){
				eqOutput[i] = audioInput[i] * eqFunction[i];
			}
		}
...
void testApp::sendTriggers()
{
	...
	for(int t=0;t<triggers.size();t++)
	{
		if(triggers[t].hit)
		{
			...
			m.clear();
			// Randomly select one of the message strings for this trigger
			int addressIndex = (int) ofRandom(0, triggers[t].addresses.size());
			cout << "sending address: " << triggers[t].addresses[addressIndex] << endl;
			m.setAddress(triggers[t].addresses[addressIndex]);
			m.addIntArg(1);
			triggers[t].sent=true;
			//lame hack ;(
			if (ofGetFrameNum() - lastFrame > 2){
				sender.sendMessage( m );
				cout<<"send "<<triggers[t].name<<endl;
			}
			...
			if(b >= triggers[t].lowBand && b <= triggers[t].highBand)
			{
				if (mode == MP3){
					if (useEQ){
						...
						if(audioInput[b]>triggers[t].height)
							triggers[t].hit=true;
					}
				} else {
					if(useEQ)
