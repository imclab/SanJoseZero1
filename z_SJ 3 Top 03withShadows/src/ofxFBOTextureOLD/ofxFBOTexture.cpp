...
void ofxFBOTexture::allocate(int w, int h, bool autoClear) {
    ...
    if (GLEE_ARB_texture_rectangle){
        texData.tex_w = w;
        texData.tex_h = h;
        texData.textureTarget = GL_TEXTURE_2D;// GL_TEXTURE_RECTANGLE_ARB; // 
    } else {
...
void *ofxFBOTexture::getPixels() {
	...
	bool autoClearState = autoClear; //LARS
	autoClear = false;
