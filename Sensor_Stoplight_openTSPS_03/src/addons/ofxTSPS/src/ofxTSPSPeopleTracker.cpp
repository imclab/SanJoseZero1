...
{	
	...
	adjustedView.setImage(colorImageWarped);//grayImageWarped);
...
void ofxTSPSPeopleTracker::updateSettings()
{
	...
	// ZACK BOKA: Update Optical Flow Threshold and Color Sensing Variance Threshold in case it changed, and update color sensing bool
	...
	colorSensingVarianceThreshold = p_Settings->colorSensingVarianceThreshold;
...
//	if (!adjustedView.isActive()) {
		...
		//	colorSmallImage.scaleIntoMe(colorImageWarped);
		//	colorBabyImage.scaleIntoMe(grayImageWarped);
...
//			}
	...
	cameraView.update(colorImage);//grayImage);
...
void ofxTSPSPeopleTracker::mousePressed( ofMouseEventArgs &e )
{
	if (isInsideRect(e.x, e.y, cameraView)){
		...
		adjustedView.setActive(); // ZACK: CHANGED
...
float* ofxTSPSPeopleTracker::getColorSensingVarianceThreshold() {
	return &colorSensingVarianceThreshold;
}
